...
export const editWorkspaceMutationFn = async ({ data, workspaceId }: EditWorkspaceType) => {
  const response = await API.put(`/workspace/update/${workspaceId}`, data);
  return response.data
};


export const deleteWorkspaceMutationFn = async (workspaceId: string): Promise<{ message: string, currentWorkspace: string }> => {
...
export const invitedUserJoinWorkspaceMutationFn = async (inviteCode: string): Promise<{ message: string, workspaceId: string }> => {
...
export const getProjectAnalyticsQueryFn = async ({ workspaceId, projectId }: ProjectByIdPayloadType) => {
...
export const getAllTasksQueryFn = async ({ workspaceId, keyword, projectId, assignedTo, priority, status, dueDate, pageNumber, pageSize }: AllTaskPayloadType): Promise<AllTaskResponseType> => {
...
export const deleteTaskMutationFn = async ({ taskId, workspaceId }: DeleteTaskType): Promise<{ message: string }> => {
  const response = await API.delete(`task/${taskId}/workspace/${workspaceId}/delete`);
  return response.data;
};


export const getTaskByIdQueryFn = async ({ taskId, projectId, workspaceId }: TaskByIdPayloadType): Promise<TaskResponseType> => {
  const response = await API.get(`task/${taskId}/project/${projectId}/workspace/${workspaceId}`);
  return response.data;
}


export const editTaskMutionFn = async ({ taskId, projectId, workspaceId, data }: EditTaskPayloadType): Promise<EditTaskResponseType> => {
